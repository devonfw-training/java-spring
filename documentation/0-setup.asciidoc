...
* In your `workspaces/main` folder run this command (NOTE: the archetypeVersion is only temporary due to a bug in `devon4j:3.2.0` release with JUnit 5 and `spring-test`):
+
```
devon java create org.example.app -DarchetypeVersion=3.1.0
```
* `cd app`
* `mvn clean install`
