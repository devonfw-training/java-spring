...
* Add the following dependencies to the `pom.xml` and then run `mvn clean install`:
[source,xml]
----
<dependency>
...
* Add this in config/application.properties and remember the value of `spring.datasource.url` you find here:
[source,txt]
----
...
* In CsrfRequestMatcher add `"/h2-console"` to String[] PATH_PREFIXES_WITHOUT_CSRF_PROTECTION. This is needed to whitelist the H2-console
